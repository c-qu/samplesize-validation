...
gerse_dgm4 <- function(n,beta){
  ...
  pin         <- exp(mu1)/(1+exp(mu1))
  term1m      <- n* pin1*(1-pin1)
  term2m      <- n* (1/2)*(1-6*pin1+6*pin1^2)*pin1*(1-pin1)*sigma1^2
  ...
  var_app_cs_mc     <-  numer/denom
  
  #fisher
  var_app_cl_mc      <- 1/(n*mean_omega)
 ...
 out               <- c(round(c(p_true,c_true),3),nevents,
                      ...
                      sqrt(var_app_cs_mc),(sqrt(var_app_cs_mc)/sqrt(var_emp_cs)-1)*100,
                      sqrt(var_emp_cl), sqrt(var_app_cl),(sqrt(var_app_cl)/sqrt(var_emp_cl)-1)*100,
                      sqrt(var_app_cl_mc),(sqrt(var_app_cl_mc)/sqrt(var_emp_cl)-1)*100)
  out
}
